...
class model_report extends CI_Model {
	...
	public function get_all_student($group_id){
		...
		// 	");
		$query = ('SELECT `join`.`order`, student.student_id, 
				student.th_prefix, student.th_firstname, 
				student.th_lastname, 
				COUNT(CASE WHEN `transaction`.SCHEDULE_schedule_id = 1 THEN 1 ELSE NULL END) AS round1, 
				COUNT(CASE WHEN `transaction`.SCHEDULE_schedule_id = 2 THEN 1 ELSE NULL END) AS round2, 
				COUNT(CASE WHEN `transaction`.SCHEDULE_schedule_id = 3 THEN 1 ELSE NULL END) AS round3 
				FROM (`student`) 
				INNER JOIN `join` ON `join`.`STUDENT_student_id`=`student`.`student_id` 
				LEFT JOIN `attend` ON `attend`.`GROUP_group_id`=`join`.`GROUP_group_id` 
				LEFT JOIN `schedule` ON `schedule`.`schedule_id`=`attend`.`SCHEDULE_schedule_id` 
				LEFT JOIN `transaction` ON `transaction`.`STUDENT_student_id`=`student`.`student_id` 
				WHERE `join`.`GROUP_group_id` = ' . $group_id . '
				GROUP BY `student`.`student_id`, `join`.`order`, `student`.`th_prefix`, `student`.`th_firstname`, `student`.`th_lastname` 
				ORDER BY `join`.`order` ASC ');
