...
class model_printbarcode extends CI_Model {
	...
	public function print_by_order($order, $group_id){
		$this->db->from('join');
		$this->db->where('order', $order);
		$this->db->where('group_id', $group_id);
		$this->db->join('student', 'student.student_id = join.STUDENT_student_id', 'inner');
		$this->db->join('group', 'group.group_id = join.GROUP_group_id', 'inner');
		return $this->db->get()->result_array();
	}
